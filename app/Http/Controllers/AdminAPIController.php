...
class AdminAPIController extends Controller
{
    ...
    public function getTravelExpense(Request $req)
    {
        ...
        if ($from_date == null && $to_date == null) 
        {
            ...
            {
                $data = TravelExpenseModel::where(['delete'=>0,'a_id'=>$a_id])->where('created_at', '>=', $date)->orderby('created_at','DESC')->get();
                foreach($data as $d)
                {
            ...
            if($role == 3)
            {
                $data = TravelExpenseModel::where(['delete'=>0,'a_id'=>$a_id])->whereDate('travel_date', '>=' ,$from_date)->whereDate('travel_date', '<=' ,$to_date)->orderby('created_at','DESC')->get();
                foreach($data as $d){
                    $u_obj=UserModel::where(['delete'=>0,'role'=>3,'is_active'=>0,'id'=>$d->a_id])->orderby('created_at','DESC')->get();
                    foreach($u_obj as $u){
                        $d->labour_name = $u->name;
    ...
    public function getExpRecord(Request $req)
    {
        ...
        foreach($sortedDates as $ed)
        {
            ...
            ->leftjoin('oa_tl_history as oth','oth.id','te.oth_id')
            ->leftjoin('sales_orders as so','so.id','oth.so_id')
            ->select('u.id as u_id','u.name as labour_name','u.delete as u_delete','u.is_active','u.emp_number','u.a_id as u_a_id','te.id','te.oth_id','te.travel_date as exp_date','te.travel_desc as exp_desc','te.mode_travel as exp_type','te.a_id','te.delete','te.attachment','te.ad_id','te.ad_remark','te.status','te.from_location','te.to_location','te.sa_remark','te.sa_id','te.travel_amount as amount','te.aprvd_amount','so.delete','so.labour','so.so_number','so.project_name','so.client_name','so.address','so.cp_name','so.cp_ph_no','so.a_id as so_aid')
    ...
    //get Technician Attendance Record
    public function techAttRecord(Request $req)
    {
        ...
        if($so_id[0] == "all"){
            ...
            {
                $p_obj=PunchInOutModel::where(['delete'=>0,'pin_u_id'=>$labours,'pin_date'=>$date])->orderby('updated_at','DESC')->get();
                ...
                else
                {
                    ...
                    $p_obj=PunchInOutModel::where(['delete'=>0,'pout_u_id'=>$labours,'pout_date'=>$date])->orderby('updated_at','DESC')->get();
    ...
    public function regulariseAttendance(Request $req)
    {
        ...
        $reg_id = $req->get('reg_id');                  // record id
    ...
    // get Technician DAILY Attendance Records
    ...
    public function updateTechTime(Request $req)
    {
        ...
        if($punchInTime != null)
        {
            ...
            if($punchInDate != $reg_date){
                return ['status' => false, 'message' => 'Punch In Date is invalid','punchInDate'=>$punchInDate,'reg_date'=>$reg_date];
            }
